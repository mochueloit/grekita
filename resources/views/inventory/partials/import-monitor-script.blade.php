    ...
    (function () {
        ...
        function renderWpLogLines(lines) {
            wpLogContent.innerHTML = lines.map(line => {
                const dimmed = line.includes('[ERROR]') || line.includes('[WARN]');
                const color = dimmed ? 'text-slate-400' : 'text-slate-300';
        ...
        async function loadWpSyncLog(importData) {
            ...
            const phaseLabel = {
                waiting: 'En espera',
                waiting_images: 'Esperando imagenes',
            ...
            wpLogSummary.textContent = wpSync.last_api_status != null
                ? `${phase}${SEP}API ${wpSync.last_api_status}${statusSuffix}`
                : `${phase}${statusSuffix}`;
            ...
            try {
                ...
                if (!response.ok) {
                    wpLogContent.innerHTML = '<p class="text-slate-500">Aun no hay actividad de WordPress.</p>';
                    return;
                }
                ...
                if (payload.history && payload.history.length > 0) {
                    ...
                    wpLogHistoryBody.innerHTML = rows.map(row => `
                        <tr>
                            ...
                            <td class="px-3 py-2 font-medium text-slate-800">${row.action ?? '-'}${row.attempt ? ' #' + row.attempt : ''}</td>
                            <td class="px-3 py-2 text-slate-700">${row.api_status ?? '-'}</td>
        ...
        function renderImageLogLines(lines) {
            ...
            if (imageLogTruncated) {
                ...
                imageLogTruncated.textContent = truncated
                    ? `Mostrando las ultimas ${displayLines.length} de ${lines.length} lineas. Descarga el .log para ver todo.`
                    : '';
            }

            imageLogContent.innerHTML = displayLines.map(line => {
                const dimmed = line.includes('[ERROR]') || line.includes('FALLO');
                const color = dimmed ? 'text-slate-400' : 'text-slate-300';
                return `<div class="${color}">${line}</div>`;
            }).join('');
        ...
        async function loadImageLog(importData) {
            ...
            try {
                ...
                if (!response.ok) {
                    imageLogContent.innerHTML = '<p class="text-slate-500">Aun no hay entradas en el log de imagenes.</p>';
                    return;
                }
                ...
                imageLogSummary.textContent = stats.total > 0
                    ...
                    : 'Sin imagenes encoladas en esta importacion.';
                ...
                if (isImageLogExpanded()) {
                    imageLogContent.innerHTML = '<p class="text-slate-500">No se pudo cargar el log de imagenes.</p>';
                }
            }
        }
        ...
        async function loadSkippedRows(importData) {
            ...
            skippedSummary.textContent = `${importData.skipped_count} fila(s) no se importaron. Revisa el motivo, corrige el CSV/Excel y vuelve a subir.`
                + (importData.skipped_csv_saved ? ' El CSV quedo guardado en el servidor.' : '');
        ...
        function renderStatsBlock(stats, importData) {
            ...
            if (importData?.import_mode === 'stock_price_xml') {
                return `
                    <p><strong>${stats.processed ?? 0}</strong> filas${SEP}
                    <strong>${stats.updated ?? 0}</strong> productos${SEP}
                    <strong>${stats.prices_updated ?? 0}</strong> precios${SEP}
                    <strong>${stats.skipped ?? 0}</strong> omitidas</p>
                    <p class="mt-1">Stock fase 2: ${stats.stock_applied ?? 0} aplicado${SEP}${stats.stock_skipped ?? 0} sin producto</p>
                `;
            }

            return `
                <p><strong>${stats.processed ?? 0}</strong> filas procesadas${SEP}
                <strong>${stats.created ?? 0}</strong> creados${SEP}
                <strong>${stats.updated ?? 0}</strong> actualizados${SEP}
                <strong>${stats.skipped ?? 0}</strong> omitidas</p>
                <p class="mt-1">Fase 2 stock: ${stats.stock_applied ?? 0} aplicado${SEP}${stats.stock_skipped ?? 0} sin producto</p>
                <p class="mt-1">Atributos: ${stats.attributes_synced ?? 0}${SEP}Imagenes en cola: ${stats.images_queued ?? 0}</p>
            `;
        }
        ...
        function renderImport(importData) {
            ...
            if (progressMode) {
                if (importData.import_mode_label) {
                    progressMode.textContent = importData.import_mode_label;
            ...
            progressStep.textContent = phaseLabel
                ? `${phaseLabel}${importData.current_step ? SEP + importData.current_step : ''}`
            ...
            if (importData.worker_hint) {
                ...
                progressStale.textContent = 'Sin actividad reciente. El worker deberia retomar en breve.';
                progressStale.classList.remove('hidden');
            }

            if (importData.queued_jobs > 0 && !importData.is_finished) {
                progressStep.textContent = (importData.current_step ? `Paso: ${importData.current_step}${SEP}` : '')
            ...
            if (importData.image_downloads) {
                ...
                if (img.total > 0 || imgPending > 0 || img.failed > 0) {
                    const finished = img.finished ? ' (imagenes terminadas)' : '';
                    progressStep.textContent = (progressStep.textContent ? progressStep.textContent + SEP : '')
                        + `Imagenes: ${img.completed ?? 0}/${img.total ?? 0} listas` + (imgPending ? `, ${imgPending} pendientes` : '') + (img.failed ? `, ${img.failed} fallidas` : '') + finished;
                }
            }
